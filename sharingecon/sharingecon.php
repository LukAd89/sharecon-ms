		...
		switch(argv(1)){
			...
			case 'enquiries':
				...
				foreach($datapast as $row){
					$tablebodypast .= '<tr><td>' . getObjectTitle($row["ObjectID"]) . '</td>' . '<td>' . getChannelName($row["OwnerID"]) . '</td>' . '<td>' . $row["LendingStart"] . '</td>' . '<td>' . $row["LendingEnd"] . '</td>' . '<td>' . $row["Rating"] . '</td>' . '<td>KLICK</td></tr>';
				}
